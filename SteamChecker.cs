...
namespace AccountChecker
{
    ...
    public class SteamChecker
    {
        ...
        {
            if (callback == null || steamUser == null)
            {
                isRunning = false;
                return;
            }
        ...
        private void OnLoggedOn(SteamUser.LoggedOnCallback callback)
        {
            ...
            {
                // Get SteamID or use 0 if null
                ulong steamId = steamUser.SteamID != null ? steamUser.SteamID : 0;
                Account = new Account(username, password, steamId, Status.Success);
            }
            else if (callback.Result == EResult.AccountLogonDenied || callback.Result == EResult.AccountLoginDeniedNeedTwoFactor)
            {
                // Get SteamID or use 0 if null
                ulong steamId = steamUser.SteamID != null ? steamUser.SteamID : 0;
                Account = new Account(username, password, steamId, Status.SteamGuardProtected);
            }
            else
            {
                Account = new Account(username, password, 0, Status.Fail);
